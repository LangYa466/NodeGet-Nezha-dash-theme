...
import { fetchSetting } from "@/lib/nodeget"
import { cn } from "@/lib/utils"
import NumberFlow, { NumberFlowGroup } from "@number-flow/react"
import { useQuery } from "@tanstack/react-query"
import { AnimatePresence, m } from "framer-motion"
import { Activity, ImageMinus } from "lucide-react"
...
function Header() {
  ...
  // 用 BASE_URL 拼绝对路径 不然在 /server/:id 这种深层路由下 "./logo.png"
  // 会被解析成 /server/logo.png  SPA 兜底返回 index.html
  const baseUrl = (import.meta as any).env?.BASE_URL || "/"
  // @ts-expect-error CustomLogo is a global variable
  const customLogo = window.CustomLogo || `${baseUrl.replace(/\/+$/, "")}/logo.png`
  ...
  useEffect(() => {
    ...
    // logo.png 是 PNG 不是 ico  type 写对  Chrome 才不会自动去请求 /favicon.ico
    link.type = "image/png"
    link.rel = "icon"

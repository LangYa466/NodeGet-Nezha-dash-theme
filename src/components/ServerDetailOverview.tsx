          ...
          <TooltipProvider delayDuration={100}>
            <Tooltip>
              <TooltipTrigger asChild>
                <Card
                  onClick={() => {
                    const fullName = countries.getName(country_code.toUpperCase(), "en") || country_code.toUpperCase()
                    window.open(`https://www.google.com/search?q=${encodeURIComponent(fullName)}`, "_blank", "noopener,noreferrer")
                  }}
                  className="rounded-[10px] bg-transparent border-none shadow-none cursor-pointer"
                >
                  <CardContent className="px-1.5 py-1">
                    <section className="flex flex-col items-start gap-0.5">
                      <p className="text-xs text-muted-foreground">{t("serverDetail.region")}</p>
                      <div className="flex items-center h-4 pl-[2px]">

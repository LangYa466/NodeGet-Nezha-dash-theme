...
// 判断 komari 上报的 GPU 名称是否代表真实存在的 GPU
// （无 GPU 的机器会上报空值或 "None"，这类不应展示 GPU 图表）
const hasRealGpu = (gpuName: unknown): boolean => {
  if (typeof gpuName !== "string") return false
  const n = gpuName.trim().toLowerCase()
  return n !== "" && n !== "none" && n !== "n/a" && n !== "null" && n !== "unknown"
}
...
export const komariToNezhaWebsocketResponse = (data: any): NezhaWebsocketResponse => {
  ...
  const servers: any[] = km_servers_cache.map((server: any) => {
    ...
    const host = {
      ...
      cpu: [server.cpu_name],
      gpu: hasRealGpu(server.gpu_name) ? [server.gpu_name] : [],
    ...
    const state = status
      ? {
          ...
          process_count: status.process || 0,
          temperatures: status.temp > 0 ? [{ Name: "CPU", Temperature: status.temp }] : [],
          gpu: hasRealGpu(server.gpu_name) && typeof status.gpu === "number" ? [status.gpu] : [],
        }
      : {
          cpu: 0,
          mem_used: 0,
  ...
  for (const [uuid, status] of statusMap.entries()) {
    const host = {
      ...
      platform_version: status.kernel_version || "",
      cpu: status.cpu_name ? [status.cpu_name] : [],
      gpu: hasRealGpu(status.gpu_name) ? [status.gpu_name] : [],
      mem_total: status.ram_total || 0,
      disk_total: status.disk_total || 0,
      swap_total: status.swap_total || 0,
      arch: status.arch || "",
    ...
    const state = {
      ...
      process_count: status.process || 0,
      temperatures: status.temp > 0 ? [{ Name: "CPU", Temperature: status.temp }] : [],
      gpu: hasRealGpu(status.gpu_name) && typeof status.gpu === "number" ? [status.gpu] : [],
    }
